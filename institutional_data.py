...
#
# Liquidations:
# - Prefer WebSocket all-market stream (!forceOrder@arr) to avoid REST overhead and bans.
#   Base WS: wss://fstream.binance.com ; raw stream at /ws/<streamName>
# =====================================================================
...
# WebSocket base (see Binance WS docs)
...
# Liquidations WS (enabled by default if include_liquidations=True)
INST_LIQ_WS_ENABLED = str(os.getenv("INST_LIQ_WS_ENABLED", "1")).strip() == "1"
INST_LIQ_MAX_WINDOW_SEC = int(float(os.getenv("INST_LIQ_MAX_WINDOW_SEC", "900")))  # keep ~15m
INST_LIQ_MIN_NOTIONAL_5M = float(os.getenv("INST_LIQ_MIN_NOTIONAL_5M", "25000"))   # scoring threshold
...
# ---------------------------------------------------------------------
# Liquidations WS state (all-market stream)
# ---------------------------------------------------------------------
_LIQ_TASK: Optional[asyncio.Task] = None
_LIQ_LOCK = asyncio.Lock()
_LIQ_EVENTS: Dict[str, "deque[Tuple[int, str, float]]"] = {}  # symbol -> deque[(ts_ms, side, notional)]
...
async def close_liquidations_stream() -> None:
    global _LIQ_TASK
...
async def close_institutional_session() -> None:
    ...
    global _SESSION
    await close_liquidations_stream()
    if _SESSION is not None and not _SESSION.closed:
        try:
...
# ---------------------------------------------------------------------
# Liquidations WS helpers
# ---------------------------------------------------------------------
def _liq_record(symbol: str, ts_ms: int, side: str, notional: float) -> None:
    ...
    dq.append((int(ts_ms), s, float(notional)))

    # prune
    cutoff = _now_ms() - int(INST_LIQ_MAX_WINDOW_SEC) * 1000
    while dq and int(dq[0][0]) < cutoff:
        dq.popleft()


def _liq_handle_one(event: Dict[str, Any]) -> None:
    try:
        o = event.get("o") or {}
        ...
        # p can be "0" on MARKET; ap is average price (when present)
        p = 0.0
        ap = 0.0
        try:
            ...
            p = 0.0
        try:
            ap = float(o.get("ap") or 0.0)
        except Exception:
            ap = 0.0

        px = ap if ap > 0 else p
        if qty <= 0 or px <= 0:
            return

        ts = int(o.get("T") or event.get("E") or _now_ms())
        notional = px * qty
        _liq_record(sym, ts, side, notional)
    except Exception:
        return


def _liq_handle_payload(payload: Any) -> None:
    ...
    if isinstance(payload, dict):
        _liq_handle_one(payload)


async def _liq_ws_loop() -> None:
    """
    Keeps one connection to:
      wss://fstream.binance.com/ws/!forceOrder@arr
    and updates _LIQ_EVENTS in-memory.
    """
    url = f"{BINANCE_FWS_BASE}/ws/!forceOrder@arr"
    backoff = 1.0

    while True:
        if _is_hard_banned() or _is_soft_blocked():
            await asyncio.sleep(1.0)
            continue
        try:
            session = await _get_session()
            async with session.ws_connect(url, heartbeat=180, autoping=True) as ws:
                LOGGER.info("[INST] Liquidations WS connected stream=!forceOrder@arr")
                backoff = 1.0
                async for msg in ws:
...
async def _ensure_liq_ws_started() -> None:
    ...
    async with _LIQ_LOCK:
        if _LIQ_TASK is not None and not _LIQ_TASK.done():
            return
        _LIQ_TASK = asyncio.create_task(_liq_ws_loop())


def get_liquidation_metrics(binance_symbol: str, window_sec: int = 300) -> Tuple[Optional[float], Optional[float]]:
    """
    Returns:
      (delta, total_notional)
      delta = (buy_notional - sell_notional) / total_notional  in [-1,+1]
    """
    try:
        ...
        for ts, side, notional in reversed(dq):
            if int(ts) < cutoff:
                break
            n = float(notional)
            if n <= 0:
        ...
        return float(delta), float(total)
    except Exception:
        return None, None
...
def _classify_flow(
    ...
    liq_delta_5m: Optional[float] = None,
    ...
    if cvd_slope is not None:
        ...
        if x <= -0.2:
            return "sell"
        return "neutral"
    if liq_delta_5m is not None and liq_notional_5m is not None and float(liq_notional_5m) > 0:
        return _classify_tape(liq_delta_5m)
    return "unknown"
...
def _score_institutional(
    ...
    *,
    ...
    liq_delta_5m: Optional[float],
    liq_notional_5m: Optional[float],
    ...
    # Liquidations (flow add-on, only if meaningful notional)
    if liq_delta_5m is not None and liq_notional_5m is not None and float(liq_notional_5m) >= float(INST_LIQ_MIN_NOTIONAL_5M):
        x = float(liq_delta_5m)
        # BUY-side liquidation orders imply short liquidations; SELL implies long liquidations.
        if b == "LONG" and x >= 0.20:
            flow_points += 1
        if b == "SHORT" and x <= -0.20:
            flow_points += 1
...
def _available_components_list(payload: Dict[str, Any]) -> List[str]:
    ...
    if payload.get("liquidations_delta_5m") is not None:
...
async def compute_full_institutional_analysis(
    ...
    *,
    ...
    # Liquidations WS metrics (optional)
    liq_delta_5m: Optional[float] = None
    liq_notional_5m: Optional[float] = None
    ...
    # Liquidations (WS)
    if include_liquidations and INST_LIQ_WS_ENABLED:
        try:
            await _ensure_liq_ws_started()
            liq_delta_5m, liq_notional_5m = get_liquidation_metrics(binance_symbol, window_sec=300)
            if liq_delta_5m is None:
                warnings.append("no_liquidations_ws_data")
        except Exception:
            warnings.append("liquidations_ws_error")
    elif include_liquidations and (not INST_LIQ_WS_ENABLED):
        warnings.append("liquidations_ws_disabled_env")
    ...
    flow_regime = _classify_flow(cvd_slope, tape_5m, liq_delta_5m=liq_delta_5m, liq_notional_5m=liq_notional_5m)
    ...
    payload: Dict[str, Any] = {
        ...
        "funding_z": funding_z,
        # liquidations
        "liquidations_delta_5m": liq_delta_5m,
        "liquidations_notional_5m": liq_notional_5m,
        # scoring debug
