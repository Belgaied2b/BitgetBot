...
# Optional .env support (non-overriding): convenient for local/dev
try:
...
# =====================================================================
# Validation (institutional hygiene)
# =====================================================================

def validate_settings() -> Tuple[bool, Tuple[str, ...]]:
    """Return (ok, issues). Never raises (safe to call at startup)."""
    issues = []

    try:
        if (not DRY_RUN) and (not API_KEY or not API_SECRET or not API_PASSPHRASE):
            issues.append("Missing API credentials while DRY_RUN=0 (API_KEY/API_SECRET/API_PASSPHRASE).")
    except Exception:
        pass

    try:
        if float(LEVERAGE) <= 0:
            issues.append("LEVERAGE must be > 0.")
    except Exception:
        pass

    try:
        if float(MARGIN_USDT) <= 0:
            issues.append("MARGIN_USDT must be > 0.")
    except Exception:
        pass

    try:
        if int(MAX_ORDERS_PER_SCAN) < 0:
            issues.append("MAX_ORDERS_PER_SCAN must be >= 0.")
    except Exception:
        pass

    try:
        if TELEGRAM_BOT_TOKEN and not TELEGRAM_CHAT_ID:
            issues.append("TELEGRAM_BOT_TOKEN set but TELEGRAM_CHAT_ID missing.")
        if TELEGRAM_CHAT_ID and not TELEGRAM_BOT_TOKEN:
            issues.append("TELEGRAM_CHAT_ID set but TELEGRAM_BOT_TOKEN missing.")
    except Exception:
        pass

    try:
        if str(POSITION_MODE or "").lower() not in ("hedge", "oneway", "one_way", "one-way"):
            issues.append("POSITION_MODE should be 'hedge' or 'oneway' for Bitget v2 semantics.")
    except Exception:
        pass

    try:
        if str(INST_MODE or "").upper() not in ("LIGHT", "NORMAL", "FULL"):
            issues.append("INST_MODE must be LIGHT|NORMAL|FULL.")
    except Exception:
        pass
