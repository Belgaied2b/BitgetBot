...
from settings import (
    ...
    DESK_EV_MODE,
    INST_LIQ_FILTER_ENABLE,
    INST_LIQ_FILTER_STRICT,
    INST_LIQ_MIN_DEPTH_USD_25BPS,
    INST_LIQ_MIN_DEPTH_BID_USD_25BPS,
    INST_LIQ_MIN_DEPTH_ASK_USD_25BPS,
...
def _inst_liquidity_guard(inst: Dict[str, Any]) -> Dict[str, Any]:
    meta: Dict[str, Any] = {
        "enabled": INST_LIQ_FILTER_ENABLE,
        "strict": INST_LIQ_FILTER_STRICT,
        "min_depth_usd_25bps": float(INST_LIQ_MIN_DEPTH_USD_25BPS),
        "min_depth_bid_usd_25bps": float(INST_LIQ_MIN_DEPTH_BID_USD_25BPS),
        "min_depth_ask_usd_25bps": float(INST_LIQ_MIN_DEPTH_ASK_USD_25BPS),
        "max_spread_bps": float(INST_LIQ_MAX_SPREAD_BPS),
    }

    spread_bps = _as_float(_pick_first(inst, ["spread_bps", "spreadBps", "spread"]))
    depth_usd = _as_float(_pick_first(inst, ["depth_usd_25bps", "depth_usd_25", "depth_25bps"]))
    depth_bid = _as_float(_pick_first(inst, ["depth_bid_usd_25bps", "depth_bid_usd_25"]))
    depth_ask = _as_float(_pick_first(inst, ["depth_ask_usd_25bps", "depth_ask_usd_25"]))

    # Extra institutional liquidity context (multi-band + composite score)
    depth_usd_10 = _as_float(_pick_first(inst, ["depth_usd_10bps", "depth_usd_10"]))
    depth_usd_50 = _as_float(_pick_first(inst, ["depth_usd_50bps", "depth_usd_50"]))
    depth_usd_100 = _as_float(_pick_first(inst, ["depth_usd_100bps", "depth_usd_100"]))

    bid_depth_10 = _as_float(_pick_first(inst, ["depth_bid_usd_10bps", "depth_bid_usd_10"]))
    bid_depth_50 = _as_float(_pick_first(inst, ["depth_bid_usd_50bps", "depth_bid_usd_50"]))
    bid_depth_100 = _as_float(_pick_first(inst, ["depth_bid_usd_100bps", "depth_bid_usd_100"]))

    ask_depth_10 = _as_float(_pick_first(inst, ["depth_ask_usd_10bps", "depth_ask_usd_10"]))
    ask_depth_50 = _as_float(_pick_first(inst, ["depth_ask_usd_50bps", "depth_ask_usd_50"]))
    ask_depth_100 = _as_float(_pick_first(inst, ["depth_ask_usd_100bps", "depth_ask_usd_100"]))

    liq_score = _as_float(_pick_first(inst, ["liquidity_score", "liq_score"]))
    liq_score_z = _as_float(_pick_first(inst, ["liquidity_score_z", "liq_score_z"]))
    depth_ratio = _as_float(_pick_first(inst, ["depth_ratio_25bps", "depth_ratio"]))

    meta.update(
        {
            "spread_bps": spread_bps,
            "depth_usd_25bps": depth_usd,
            "depth_bid_usd_25bps": depth_bid,
            "depth_ask_usd_25bps": depth_ask,
            "depth_usd_10bps": depth_usd_10,
            "depth_usd_50bps": depth_usd_50,
            "depth_usd_100bps": depth_usd_100,
            "bid_depth_10bps": bid_depth_10,
            "bid_depth_50bps": bid_depth_50,
            "bid_depth_100bps": bid_depth_100,
            "ask_depth_10bps": ask_depth_10,
            "ask_depth_50bps": ask_depth_50,
            "ask_depth_100bps": ask_depth_100,
            "liquidity_score": liq_score,
            "liquidity_score_z": liq_score_z,
...
def _inst_micro_filters(inst: Dict[str, Any], bias: str, setup_intent: str) -> Tuple[bool, List[str], Dict[str, Any]]:
    ...
    liq_meta = _inst_liquidity_guard(inst)
    meta["liquidity"] = liq_meta
    meta["liquidity_risk_factor"] = float(liq_meta.get("risk_factor", 1.0))
    if not liq_meta.get("ok", True):
        vetoes.append(f"liquidity:{liq_meta.get('reason')}")

    if not INST_MICRO_FILTER_ENABLE:
        hard_ok = not bool(liq_meta.get("hard_veto"))
        return hard_ok, vetoes, meta
